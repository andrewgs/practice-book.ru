...
<!--[if (gt IE 9)|!(IE)]><!--> <body> <!--<![endif]-->
			...
			<div class="container_12 framing">
				<div class="grid_12">
					...
					<div class="rep-container">
						<?php $this->load->view('forms/frmtipslist'); ?>
						<div id="frmInsPitFalls" style="display:none;">
							<?php $this->load->view('forms/frminstips');?>
						</div>
						<button id="insTips" style="height:2.5em; margin-top:15px; min-width: 130px;">
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("#insTips").click(function(){
				$("#validError").text('');
				if($("#formAddTips").is(":hidden")){
					$("#insTips").html('<img src="<?=$baseurl;?>images/arrow-curve.png"><font size="3"> Отменить</font>');
					$("#frmInsPitFalls").slideDown("slow");
					var height = ($(window).height()*$(".box").size())/2;
					$("html, body").animate({scrollTop:height+'px'},"slow");
				}else{
					$("#formAddTips").slideUp("slow",function(){
						$("#formAddTips").hide();
						$("#insTips").html('<img src="<?=$baseurl;?>images/news-plus.png"><font size="3"> Добавить</font>');
			...
			$(".NewsDel").click(function(){
				...
				$.post(
					"<?=$baseurl;?>managers/delete-tips/<?=$userinfo['uconfirmation'];?>",
					{'id':newsID},
					function(data){
						if(data.status){
			...
			$(".NewsSave").click(function(){
				...
				if(err){
					...
					$.post("<?=$baseurl;?>managers/save-tips/<?=$userinfo['uconfirmation'];?>",

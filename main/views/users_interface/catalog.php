	...
	<div id="container" class="main-wrap">
		...
		<div id="main" class="whitebg">
			<div class="contentblock">
				<div class="work">
					<div class="work-title"><h3>Каталог отраслей по городу Москва</h3></div>
					<div class="work-text">
						<?=anchor('catalog','В начало');?>
						<hr/>
						<?php for($i=0;$i<count($actlist);$i++):?>
						<dl>
							<?php if($actlist[$i]['act_final']):?>
								<dt><?=anchor('activity-information/region/11/activity/'.$actlist[$i]['act_id'],$actlist[$i]['act_title'].' (это ссылка на страницу отрасли)',array('target'=>'_blank'));?></dt>
							<?php else:?>
								<dt><?=anchor('catalog/activity/'.$actlist[$i]['act_id'],$actlist[$i]['act_title'].' (ссылка на подотрасль)');?></dt>
							<?php endif;?>
						</dl>
						<?php endfor;?>
					</div>

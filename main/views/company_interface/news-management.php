		...
		<div id="main">
			<div class="container_12 framing">
				<div class="grid_12">
					...
					<h4 style="margin-top: 15px;">Новости компании</h4>
		...
		$(document).ready(function(){
			...
			$(".ExtendDay").click(function(){
				...
				if(valExtend == ''){
					...
					$.post(
					"<?=$baseurl;?>company/news-extend-day/<?=$userinfo['uconfirmation'];?>",
			...
			$(".NewsEdit").click(function(){
				...
				$(objDescID).html("<textarea class=\"edit-form-textarea mbottom\" name=\"description\" id=\"ads"+curID+"\" cols=\"50\" rows=\"12\">"+valDesc+"</textarea>");
			...
			$(".NewsDel").click(function(){
				...
				var newsID = $("#id"+curID).text();
				$.post(
					"<?=$baseurl;?>company/news-delete/<?=$userinfo['uconfirmation'];?>",
					{'id':newsID},
					function(data){
						if(data.status){
			...
			$(".NewsSave").click(function(){
				...
				if(err){
					...
					$.post("<?=$baseurl;?>company/news-save/<?=$userinfo['uconfirmation'];?>",

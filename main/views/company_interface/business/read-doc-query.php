...
<?=anchor('business-environment/documentation/'.$userinfo['uconfirmation'].'/delete-query/'.$topic['dtt_id'],'Удалить',array('title'=>'Удалить'));?>
				...
				<td><?=$topic['usubname'].' '.$topic['uname'].' '.$topic['uposition'].' '.$topic['cmp_name']?></td>
								...
								<div class="right-comment">
									<a name="comments"></a>
									<div class="right-comment-title">комментарии (<?=$topic['dtt_comments'];?>):</div>
...
<?=anchor('business-environment/documentation/'.$userinfo['uconfirmation'].'/document-query/'.$this->uri->segment(5).'/edit-comment/'.$comments[$i]['cmn_id'],'Редактировать');?>
<?=anchor('business-environment/documentation/'.$userinfo['uconfirmation'].'/document-query/'.$this->uri->segment(5).'/delete-comment/'.$comments[$i]['cmn_id'],'Удалить');?>
											</div>
										<?php endif;?>
										</div>
									</div>

						...
						<div class="content-right-bot">
							...
							<?=anchor($backpath,'Вернуться назад',array('class'=>'lnk-submit'));?>
								...
								<div class="right-post">
									...
									<span class="green"><a href="" id="SetComment">ответить</a></span>
									<div class="clear">&nbsp;</div>
									<div id="FormComment" style="display:none;">
										<?=form_open($this->uri->uri_string(),array('class'=>'formular')); ?>
					<label class="label-input">Текст ответа: <span class="necessarily" title="Поле не может быть пустым">*</span></label>
									<?= form_error('note'); ?>
...
<?=anchor('business-environment/question-answer/'.$userinfo['uconfirmation'].'/delete-question/'.$question['qat_id'],'Удалить',array('title'=>'Удалить'));?>
				...
				<td><?=$question['usubname'].' '.$question['uname'].'<br/>'.$question['uposition'].'<br/>'.$question['cmp_name']?></td>
														</tr>
													</table>
												</td>
											</tr>
...
<?=anchor('business-environment/question-answer/'.$userinfo['uconfirmation'].'/question/'.$this->uri->segment(5).'/delete-answer/'.$comments[$i]['cmn_id'],'Удалить');?>
												<div class="clear">&nbsp;</div>

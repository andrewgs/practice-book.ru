			...
			<div class="contentblock">
				...
				<div class="content-right">
					...
					<img src="<?=$baseurl.$type_news;?>/viewimage/<?=$topic['ben_id'];?>"class="floated" width="74" height="74" alt=""/>
									...
									<p><?=$topic['ben_note'];?></p>
									<?php if($topic['ben_source']):?>
										<p>Источник: <?=$topic['ben_source'];?></p>
									<?php endif;?>
									<span class="date"><?=$topic['ben_date'];?></span>
									<span class="view">просмотров: <?=$topic['ben_views'];?></span>
									<span class="green">
			<?=anchor('business-environment/'.$type_news.'/'.$userinfo['uconfirmation'].'/news/'.$topic['ben_id'].'/comment','ответить');?>
									</span>
									<div class="clear">&nbsp;</div>
										...
										<table cellspacing="0" class="post-option">
											<tr>
												<td class="right-option">
													<div class="opt-bg">
													<?php if($topic['ben_userid'] == $userinfo['uid']):?>
														<div class="opt-bgg">
<?=anchor('business-environment/'.$type_news.'/'.$userinfo['uconfirmation'].'/edit-news/'.$topic['ben_id'],'Редактировать',array('class'=>'first','title'=>'Редактировать'));?>
<?=anchor('business-environment/'.$type_news.'/'.$userinfo['uconfirmation'].'/track-news/'.$topic['ben_id'],'Отслеживать',array('title'=>'Отслеживать'));?>
<?=anchor('business-environment/'.$type_news.'/'.$userinfo['uconfirmation'].'/share-news/'.$topic['ben_id'],'Поделиться',array('title'=>'Поделиться'));?>
<?=anchor('business-environment/'.$type_news.'/'.$userinfo['uconfirmation'].'/delete-news/'.$topic['ben_id'],'Удалить',array('title'=>'Удалить'));?>
														</div>
													<?php endif; ?>
													</div>
												</td>
												<td class="right-avtor">
													<table cellspacing="0" cellpadding="0">
														<tr>
				<td><img src="<?=$baseurl;?>cravatar/viewimage/<?=$topic['ben_userid'];?>" alt="" align="left" width="42" height="42"/></td>
				<td><?=$topic['usubname'].' '.$topic['uname'].' '.$topic['uposition'].' '.$topic['cmp_name']?></td>
								...
								<div class="right-comment">
									<a name="comments"></a>
									<div class="right-comment-title">комментарии (<?=$topic['ben_comments'];?>):</div>
...
<?=anchor('business-environment/'.$type_news.'/'.$userinfo['uconfirmation'].'/news/'.$this->uri->segment(5).'/edit-comment/'.$comments[$i]['cmn_id'],'Редактировать');?>
<?=anchor('business-environment/'.$type_news.'/'.$userinfo['uconfirmation'].'/news/'.$this->uri->segment(5).'/delete-comment/'.$comments[$i]['cmn_id'],'Удалить');?>
											</div>
										<?php endif;?>
										</div>
									</div>

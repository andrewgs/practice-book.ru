...
<?=anchor('business-environment/associations/'.$userinfo['uconfirmation'].'/create-section','<img src="'.$baseurl.'images/add_events.png" alt="Создать тему"/>',array('title'=>'Создать тему'));?>
				...
				<div class="content-right">
					...
					<label class="label-input">Название: <span class="necessarily" title="Поле не может быть пустым">*</span></label>
									<?=form_error('title');?>
				<input class="edit450-form-input inpv" id="title" maxlength="150" name="title" type="text" value="<?=set_value('title');?>"/>
									<div class="clear"></div>
					...
					<label class="label-input">Цена: <span class="necessarily" title="Поля не можут быть пустыми">*</span> (Пример заполнения: 5000 руб.)</label>
									<?=form_error('price');?>
				<input class="small150-form-input inpv" id="price" maxlength="50" name="price" type="text" value="<?=set_value('price');?>"/>
									<div class="clear"></div>
				...
				<input class="small150-form-input inpv" id="must1" maxlength="50" name="must1" type="text" value="<?=set_value('must1');?>"/>
									...
									<div id="dregion" style="float:right;">
										<div class="box">
											<div class="box-header">
												<h3>Регионы <span class="necessarily" title="Поле не может быть пустым">*</span></h3></div>
												<div class="box-content w280 h250">
													...
													<?php for($i=0;$i<count($regions);$i++): ?>
														...
														<?php while($regions[$j]['reg_district'] === $regions[$i]['reg_district']): ?>
															<option value="<?=$regions[$j]['reg_id'];?>"
																<?=set_select('region[]',$regions[$j]['reg_id']);?>>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("#addAssociation").click(function(event){
				var err = false;

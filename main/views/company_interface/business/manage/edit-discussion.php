			...
			<div class="contentblock">
						...
						<div class="content-left-text">
							<div class="left-menu">
								...
								<?php for($i=0;$i<count($sections);$i++): ?>		
									<li><?=anchor('business-environment/discussions/'.$userinfo['uconfirmation'].'/section/'.$sections[$i]['dsc_id'],$sections[$i]['dsc_title'],array('id'=>'link'.$sections[$i]['dsc_id']));?></li>
								<?php endfor; ?>
								</ul>
							</div>
						</div>
				...
				<div class="content-right">
					...
					<label class="label-input">Название обсуждения: <span class="necessarily" title="Поле не может быть пустым">*</span></label>
									<?= form_error('title'); ?>
					<input class="edit450-form-input" id="title" maxlength="150" name="title" type="text" value="<?=$topic['top_title'];?>"/>
									<div class="clear"></div>
					<label class="label-input">Содержание: <span class="necessarily" title="Поле не может быть пустым">*</span></label>
									<?= form_error('note'); ?>
					<textarea class="edit700-form-textarea" name="note" id="note" cols="50" rows="10"><?=$topic['top_note'];?></textarea>
									<div class="clear"></div>

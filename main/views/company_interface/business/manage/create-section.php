		...
		<div id="main" class="whitebg">
			<div class="contentblock">
				<div class="content-top">
					...
					<h1>Business Environment (Бизнес-Среда): <?=$envname;?></h1>
				</div>
						...
						<div class="content-left-text">
							<div class="left-menu">
								...
								<?php for($i=0;$i<count($sections);$i++): ?>		
									<li><?=anchor('business-environment/'.$type.'/'.$userinfo['uconfirmation'].'/section/'.$sections[$i][$field.'id'],$sections[$i][$field.'title']);?></li>
								<?php endfor; ?>
								</ul>
							</div>
						</div>
							...
							<?=anchor($backpath,'Вернуться назад',array('class'=>'lnk-submit'));?>
								...
								<?=form_open($this->uri->uri_string(),array('id'=>'formAddSection','class'=>'formular')); ?>
									<label class="label-input">
										Название темы: <span class="necessarily" title="Поле не может быть пустым">*</span>
									</label>
									<?= form_error('title'); ?>
						<input class="edit450-form-input" id="title" maxlength="50" name="title" type="text" value="<?=set_value('title');?>"/>
						<input class="btn-action margin-1em" id="addSection" type="submit" name="submit" value="Добавить"/>
								<?= form_close(); ?>
								</div>
							</div>
						</div>

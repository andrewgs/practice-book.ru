...
<!--[if (gt IE 9)|!(IE)]><!--> <body> <!--<![endif]-->
			...
			<section id="frmlogin"><br/>
				<div class="container_12">
					...
					<div class="grid_2">
						<h2>&nbsp;</h2>
						<h2>&nbsp;</h2>
						<input class="btn-action" style="margin-top:0" id="setSearch" type="button" name="submit" value="Открыть отрасль"/>
					</div>
					<div class="clear"></div>
					<hr size="2"/>
				</div>
	...
	$(document).ready(function(){
		...
		$("#setSearch").click(function(){var actID = $("#ASV").text();if(actID != ''){if(!confirm("Сейчас будет произведено открытие отрали. Продолжить?")) return false;window.setTimeout("window.location='<?=$baseurl;?>admin/create-activity-regions/<?=$userinfo['uconfirmation'];?>/activity/"+actID+"'",1000);}});
		function msgerror(msg){$.blockUI({message: msg,css:{border:'none',padding:'15px', size:'12.0pt',backgroundColor:'#000',color:'#fff',opacity:'.8','-webkit-border-radius': '10px','-moz-border-radius': '10px'}});window.setTimeout($.unblockUI,5000);return false;}
	});
	</script>
</body>
</html>
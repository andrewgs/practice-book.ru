<?php if(count($specials)): ?>
	...
	<div class="box" id="cs<?=$i?>">
		...
		<div class="box-content h150 w918">
			<div class="newsID" id="id<?=$i?>"><?=$specials[$i]['spc_id'];?></div>
			<div class="content-separator">
				<div class="nshDate"><?=$specials[$i]['spc_date'];?></div>
				<img src="<?=$baseurl;?>specials/viewimage/<?=$specials[$i]['spc_id'];?>" class="floated" id="img<?=$i?>" alt=""/>
				<div class="nsh-title" id="t<?=$i?>"><?=$specials[$i]['spc_title'];?></div>
				<div class="nshNote" id="ds<?=$i?>"><?=$specials[$i]['spc_note'];?></div>
				<div id="src<?=$i?>">
					<span class="text">Источник: </span>
					<div class="nsh-title"><?=$specials[$i]['spc_source'];?></div>
				</div>
